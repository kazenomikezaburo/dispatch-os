...
const all: AttendanceQuery = { date: "2026-08-22", shift: "", state: "all", confirmation: "all", attention: "all", q: "", page: 1 };
...
const dataSource = readFileSync("lib/admin/attendance/get-attendance.ts", "utf8"); record("035", (dataSource.match(/\.from\(/g) ?? []).length === 4 && ["attendance_events", "attendance_records", "attendance_record_revisions"].every((table) => dataSource.includes(`from("${table}")`)) && (dataSource.match(/\.in\("assignment_id", ids\)/g) ?? []).length === 3, "one date-bounded Assignment query plus three ID-batched Attendance queries");
