...
export function ShiftEditDrawer({ detail }: { detail: AdminShiftDetail }) {
  ...
  return <>
    <button type="button" onClick={() => { setSaved(false); setConflict(false); setOpen(true); }} className="inline-flex min-h-11 items-center rounded-md bg-primary px-4 text-sm font-semibold text-primary-foreground hover:bg-primary-hover focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-info">シフトを編集</button>
    {saved && <AdminFeedback kind="success" message="シフトを保存しました。" />}
    <Drawer open={open} titleId="shift-edit-title" closeDisabled={pending} onClose={close}>
      ...
      {open && <div className="p-4 sm:p-6">{conflict && <button type="button" onClick={() => { setOpen(false); setConflict(false); router.refresh(); }} className="mb-4 inline-flex min-h-11 items-center gap-2 rounded-md border border-border px-4 text-sm font-semibold text-link"><RefreshCw aria-hidden="true" className="size-4" />最新の内容を読み込む</button>}<p className="mb-4 text-sm text-foreground-secondary">このシフト1件のみを編集します。他の日付には反映されません。</p><ShiftForm key={detail.updatedAt} mode="edit" options={options} initialValues={initialValues} shiftId={detail.id} expectedUpdatedAt={detail.updatedAt} restrictions={detail.editRestrictions} submitAction={submitAction} onConflict={() => setConflict(true)} onCancel={close} onPendingChange={setPending} onSuccess={() => { setPending(false); setSaved(true); setOpen(false); router.refresh(); }} /></div>}
    </Drawer>
  </>;
}
