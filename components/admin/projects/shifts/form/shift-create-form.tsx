...
export function ShiftForm({ mode = "create", options, initialValues, expectedUpdatedAt, shiftId, cancelHref, onCancel, submitLabel = mode === "edit" ? "変更を保存" : "シフトを追加", pendingLabel = mode === "edit" ? "保存中..." : "シフトを追加中...", showContext = true, submitAction = createShift, onSuccess, onConflict, onPendingChange, restrictions }: ShiftFormProps) {
  ...
  const { register, handleSubmit, reset, setError, formState: { errors, isSubmitting } } = useForm<ShiftFormValues>({ resolver: zodResolver(shiftFormSchema), defaultValues: { start_date: "", start_time: "", end_date: "", end_time: "", required_workers: "1", break_minutes: "", deadline_date: "", deadline_time: "", status: defaultStatus, ...initialValues } });
