...
import { projectEditHref } from "@/components/admin/editor/admin-editor-contract";
...
export function ProjectDetailHeader({ project }: { project: ProjectDetail }) {
  return <header className="space-y-5">
    ...
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      ...
      <div className="flex min-h-11 flex-wrap items-center gap-3 sm:max-w-sm sm:justify-end"><ProjectStatusBadge status={project.status} /><Link href={projectEditHref(project.id)} className="inline-flex min-h-11 items-center justify-center rounded-control bg-primary px-4 text-sm font-semibold text-primary-foreground hover:bg-primary-hover focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring">案件を編集</Link></div>
    </div>
  </header>;
}
