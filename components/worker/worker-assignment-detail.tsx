...
import { HelpRequestSection } from "./help-request/help-request-section";
...
export function WorkerAssignmentDetail({ assignment }: { assignment: WorkerAssignment }) {
  ...
  return <main className="mx-auto max-w-3xl space-y-6 px-4 py-6 sm:px-6 sm:py-10">
    ...
    <HelpRequestSection assignmentId={assignment.id} canCreate={assignment.canCreateIncident} incidents={assignment.incidents} context={{ date: day.format(new Date(assignment.startsAt)), time: `${time.format(new Date(assignment.startsAt))}〜${time.format(new Date(assignment.endsAt))}`, workplace: assignment.workplaceName, job: assignment.jobName }} />
