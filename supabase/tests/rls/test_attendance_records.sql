...
select test_rls.assert_allowed('RLS-AR-004B', 'Critical', $$insert into public.attendance_records (id, assignment_id, planned_start_at, planned_end_at) values ('70000000-0000-0000-0000-000000000092','40000000-0000-0000-0000-000000000005','2099-01-16 09:00:00+09','2099-01-16 18:00:00+09')$$, 'INSERT', 'Nagoya attendance record');
select test_rls.assert_allowed('RLS-AR-004C', 'Critical', $$update public.attendance_records set status = 'working' where id = '70000000-0000-0000-0000-000000000002'$$, 'UPDATE', 'Nagoya attendance record');
...
select test_rls.assert_allowed('RLS-AR-006B', 'Critical', $$insert into public.attendance_records (id, assignment_id, planned_start_at, planned_end_at) values ('70000000-0000-0000-0000-000000000093','40000000-0000-0000-0000-000000000005','2099-01-16 09:00:00+09','2099-01-16 18:00:00+09')$$, 'INSERT', 'attendance record');
select test_rls.assert_allowed('RLS-AR-006C', 'Critical', $$update public.attendance_records set status = 'working' where id = '70000000-0000-0000-0000-000000000003'$$, 'UPDATE', 'Tokyo attendance record');
