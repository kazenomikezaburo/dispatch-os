-- OCV1-02B: bounded, Worker-owned recruitment discovery projection.
-- This intentionally does not expose the STAFF-2F Admin candidate reader.

create function public.get_own_recruitment_shifts(
...
begin
  ...
  )) order by starts_at, id), '[]'::jsonb) into v_items from (select * from rendered limit v_limit) r;

  return jsonb_build_object('ok', true, 'items', v_items, 'hasMore', (select count(*) > v_limit from candidate));
end;
$$;
