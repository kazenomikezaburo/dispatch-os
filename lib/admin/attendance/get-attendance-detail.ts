...
type AssignmentRow = { id: string; shift_slot_id: string; worker_id: string; status: string; workers: { display_name: string }; shift_slots: { starts_at: string; ends_at: string; break_minutes: number | null; jobs: { name: string; projects: { id: string; name: string }; workplaces: { name: string } } } };
...
export async function getAttendanceDetail(assignmentId: string): Promise<AttendanceDetailResult> {
  try {
    ...
    const assignmentResult = await supabase.from("assignments").select(`id, shift_slot_id, worker_id, status, workers!inner(display_name), shift_slots!inner(starts_at, ends_at, break_minutes, jobs!inner(name, projects!inner(id, name), workplaces!inner(name)))`).eq("id", assignmentId).maybeSingle();
