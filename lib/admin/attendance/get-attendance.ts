...
export async function getAttendance(query: AttendanceQuery, now = new Date()): Promise<AttendanceResult> {
  try {
    ...
    const [events, records, revisions] = await Promise.all([
      supabase.from("attendance_events").select("assignment_id, event_type, server_received_at").in("assignment_id", ids).in("event_type", ["start_work", "end_work"]).order("server_received_at"),
      supabase.from("attendance_records").select("assignment_id").in("assignment_id", ids),
      supabase.from("attendance_record_revisions").select("assignment_id").in("assignment_id", ids),
    ]);
    if (events.error || records.error || revisions.error) throw events.error ?? records.error ?? revisions.error;
    ...
    const corrected = new Set((revisions.data ?? []).map((revision) => revision.assignment_id));
    const confirmations = new Map<string, "confirmed" | "corrected">((records.data ?? []).map((record) => [record.assignment_id, corrected.has(record.assignment_id) ? "corrected" : "confirmed"]));
    return { ok: true, data: buildAttendanceData(inputs, query, now, confirmations) };
