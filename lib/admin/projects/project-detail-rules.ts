...
export type DetailProjectInput = ProjectInput & { branchId: string; description: string | null };
...
export function buildProjectDetail(project: DetailProjectInput, jobs: DetailJobInput[], activeAssignmentShiftIds: string[]): ProjectDetail {
  ...
  const detailJobs: ProjectDetailJob[] = jobs.map((job) => {
    ...
    return { id: job.id, name: job.name, status: job.status, workplace: job.workplace, shifts, shiftCount: shifts.length, requiredWorkers, assignedWorkers, shortage: Math.max(requiredWorkers - assignedWorkers, 0) };
  ...
  return { id: project.id, name: project.name, branchId: project.branchId, clientName: project.clientName, status: project.status, startDate: project.startDate, endDate: project.endDate, description: project.description, summary, jobs: detailJobs };
}
