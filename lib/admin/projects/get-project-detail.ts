...
export async function getProjectDetail(projectId: string): Promise<ProjectDetailResult> {
  try {
    ...
    const projectResult = await supabase.from("projects").select("id, branch_id, client_id, name, status, start_date, end_date, description, updated_at, clients(name)").eq("id", projectId).maybeSingle();
