...
export type PreShiftQuery = { date: string; q: string; project: string; shift: string; status: PreShiftStatus; assignment: string };
...
export function parsePreShiftQuery(raw: Record<string, string | string[] | undefined>, now = new Date()): PreShiftQuery {
  ...
  return { date: isShiftDate(date) ? date : tomorrow, q: first(raw.q).trim().slice(0, 100), project: first(raw.project), shift: first(raw.shift).slice(0, 100), status: status === "pending" || status === "confirmed" ? status : "all", assignment: first(raw.assignment) };
}
...
export function filterPreShiftItems(items: PreShiftItem[], query: PreShiftQuery) {
  ...
  return items.filter((item) => (!needle || `${item.workerName} ${item.projectName}`.toLocaleLowerCase("ja").includes(needle)) && (!query.project || item.projectId === query.project) && (!query.shift || item.shiftId === query.shift) && (query.status === "all" || (query.status === "confirmed") === Boolean(item.confirmation))).sort((a,b) => Number(Boolean(a.confirmation)) - Number(Boolean(b.confirmation)) || a.startsAt.localeCompare(b.startsAt) || a.workerName.localeCompare(b.workerName,"ja") || a.assignmentId.localeCompare(b.assignmentId));
}
export function preShiftHref(query: PreShiftQuery, patch: Partial<PreShiftQuery> = {}, basePath = "/admin/pre-shift") {
  const next = { ...query, ...patch }; const params = new URLSearchParams({ date: next.date });
  if (next.q) params.set("q", next.q); if (next.project) params.set("project", next.project); if (next.shift) params.set("shift", next.shift); if (next.status !== "all") params.set("status", next.status); if (next.assignment) params.set("assignment", next.assignment);
