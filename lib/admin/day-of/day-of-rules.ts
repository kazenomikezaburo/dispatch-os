...
export function parseDayOfQuery(raw:Record<string,string|string[]|undefined>,now=new Date()):DayOfQuery {
  ...
  return {date:isShiftDate(date)?date:tokyoDate(now),q:first(raw.q).trim().slice(0,100),project:first(raw.project),shift:first(raw.shift).slice(0,100),state:(["attention","scheduled","working","finished"].includes(state)?state:"all") as DayOfFilter,assignment:first(raw.assignment)};
}
...
export function buildDayOfItems(inputs:DayOfInput[],query:DayOfQuery,now=new Date()):DayOfItem[]{
  ...
  return items.filter((item)=>(!needle||[item.workerName,item.staffCode,item.projectName,item.jobName,item.workplaceName].some(v=>v.toLocaleLowerCase("ja").includes(needle)))&&(!query.project||item.projectId===query.project)&&(!query.shift||item.shiftId===query.shift)&&matchState(item)).sort((a,b)=>priority(a)-priority(b)||a.startsAt.localeCompare(b.startsAt)||a.workerName.localeCompare(b.workerName,"ja")||a.assignmentId.localeCompare(b.assignmentId));
}
