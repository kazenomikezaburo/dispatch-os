...
export default async function NewShiftPage({ params }: PageProps<"/admin/projects/[projectId]/jobs/[jobId]/shifts/new">) {
  await connection(); const values = await params; const projectId = idSchema.safeParse(values.projectId); const jobId = idSchema.safeParse(values.jobId); if (!projectId.success || !jobId.success) notFound();
