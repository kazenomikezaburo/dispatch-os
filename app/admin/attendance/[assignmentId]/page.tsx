...
export default async function AttendanceDetailPage({ params }: PageProps<"/admin/attendance/[assignmentId]">) {
  ...
  if (!result.ok) return <AdminPage><section role="alert" className="rounded-lg border border-red-200 bg-white p-5"><h1 className="font-semibold">勤怠詳細を取得できませんでした。</h1><p className="mt-1 text-sm text-slate-600">時間をおいて再度お試しください。</p></section></AdminPage>;
